...
const columns: ColumnDef<Business>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      ...
      >
        {row.original.metadata.status.charAt(0).toUpperCase() +
          row.original.metadata.status.slice(1)}
      </Badge>
    ),
  },
  {
...
export function BusinessesDataTable({ data, onView, onEdit, onDelete }: BusinessesDataTableProps) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="w-full text-center py-8 text-muted-foreground">No businesses found.</div>
    );
  }
