...
describe("Auth Schemas", () => {
  ...
  describe("passwordSchema", () => {
    it("validates strong passwords", () => {
      const validPasswords = ["Password1", "MyP@ssw0rd", "Test1234", "Abc12345"];
    ...
    it("rejects passwords without lowercase letters", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((issue) => issue.message.includes("one lowercase letter")),
        ).toBe(true);
      }
    });
    ...
    it("rejects passwords without uppercase letters", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((issue) => issue.message.includes("one uppercase letter")),
        ).toBe(true);
      }
    });
  ...
  describe("nameSchema", () => {
    it("validates correct names", () => {
      const validNames = ["John Doe", "María García", "José Rizal", "Test User 123"];
